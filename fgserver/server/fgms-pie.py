...
from data.comms import ChatMessage, CommFrequency
...
def make_fgms_packet(sender_callsign, packet_type, content_data):
    ...
    packet.pack_int(settings.radar_range) # Visibility range (was: ReplyAddress; see message 35687340 on the FG devel list)
...
def mkFgmsMsg_position(callsign, aircraft_model, pos_coords, pos_amsl, hdg=0, pitch=0, roll=0, properties={}, legacy=False):
    ...
    '''
    buf = PacketData()
    buf.pack_padded_string(96, aircraft_model) # Aircraft model
    ...
    return make_fgms_packet(callsign, position_message_type_code, buf).allData()
...
class FGMShandshaker(QThread):
    ...
    def run(self):
        ...
        if settings.publicised_frequency != None:
            pdct[FGMS_prop_comm_freq] = int(1000000 * settings.publicised_frequency.MHz()) if settings.transmitting_radio else 0
            pdct[FGMS_prop_ATCpie_publicised_freq] = str(settings.publicised_frequency)
        pos = env.radarPos()
        packet = mkFgmsMsg_position(self.callsign, ATCpie_model_string, pos, env.elevation(pos),\
                properties=pdct, legacy=settings.FGMS_legacy_protocol)
        #DEBUG('Sending packet with size %d=0x%x bytes. Optional data is: %s' % (len(packet), len(packet), packet.data[228:]))
        try:
...
def decode_FGMS_position_message(packet):
    ...
    - position received in header: (EarthCoords, geometric altitude AMSL in ft)
    - transponder status dict of squawked values
    - the packed text chat message line if any
    ...
    got_chat_line = got_xpdr_capability = got_transmission_freq = None # from regular (non-generic) MP properties
    ...
    while len(buf) >= 4:
        try:
            prop_code, prop_value = buf.unpack_property()
            if prop_code == FGMS_prop_chat_msg:
                got_chat_line = prop_value
            elif prop_code == FGMS_prop_comm_freq and prop_value != 0:
                got_transmission_freq = CommFrequency(prop_value / 1000000)
                ...
                res_xpdr_data[Xpdr.ALT] = StdPressureAlt(prop_value)
                ...
                if prop_code == FGMS_prop_ATCpie_social_name:
                    ...
                    got_publicised_frq = CommFrequency(prop_value)
                elif prop_code == FGMS_prop_ATCpie_version_string:
                    got_version_string = prop_value
            last_prop_OK = prop_code
        except ValueError as err:
            aux = 'none read before it' if last_prop_OK == None else 'last OK had code %d' % last_prop_OK
            #DEBUG print('Problem reading property from %s (%s): %s' % (got_callsign, aux, err))
    #DEBUG if len(buf) != 0:
...
class FgmsAircraft(Aircraft):
    ...
    def digestChatLine(self, msg):
        if msg != self.last_chat_message_read:
            self.last_chat_message_read = msg
            signals.incomingRadioChatMsg.emit(ChatMessage(self.identifier, msg))
    
    def digestRadioTransmission(self, frq):
        if frq == None and env.rdf.receivingSignal(self.identifier): # aircraft stopped transmitting
            env.rdf.dieSignal(self.identifier)
        elif frq != None and any(f.inTune(frq) for f in settings.MP_RDF_frequencies.values()) \
                and (self.last_radio_transmit_freq == None or not frq.inTune(self.last_radio_transmit_freq)):
            # aircraft beginning transmission on (or switched to) my publicised frequency
            env.rdf.receiveSignal(self.identifier, self.liveCoords, timeOut=(FGMS_connection_timeout / 2))
            # NOTE: timeout above is to avoid call to liveCoords after aircraft is deleted
...
def update_FgmsAircraft_list(ACFT_lst, udp_packet):
    ...
    if chat_msg != None and chat_msg != '':
        fgms_acft.digestChatLine(chat_msg)
    if radio_transmission != None:
        if env.radarPos().distanceTo(pos_coords) > pos_alt / 100 + los_min_dist: # ACFT too far
            ...
            radio_transmission = None
    fgms_acft.digestRadioTransmission(radio_transmission)
